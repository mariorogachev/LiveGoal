...
const SportsTabs = () => {
  ...
  return (
    <div className="flex flex-col md:flex-row px-28 pt-32 md:px-32 md:pt-32 h-screen bg-gray-100 dark:bg-gray-900">
      <div className="bg-gray-800 text-white p-4 md:w-1/4 dark:border border-solid">
        <h1 className="text-2xl font-semibold mb-4">Sports Tabs</h1>
        <div>
          <h2 className="text-lg font-semibold mb-2">Sports</h2>
          <button className={`font-mono h-20 w-full py-2 px-4 mb-2 text-center ${selectedSport === 'Football' ? 'bg-gray-700' : 'hover:bg-gray-700'} focus:outline-none focus:shadow-outline`} onClick={() => changeSport('Football')}>Football</button>
          <button className={`font-mono h-20 w-full py-2 px-4 mb-2 text-center ${selectedSport === 'Basketball' ? 'bg-gray-700' : 'hover:bg-gray-700'} focus:outline-none focus:shadow-outline`} onClick={() => changeSport('Basketball')}>Basketball</button>
          <button className={`font-mono h-20 w-full py-2 px-4 mb-2 text-center ${selectedSport === 'Tennis' ? 'bg-gray-700' : 'hover:bg-gray-700'} focus:outline-none focus:shadow-outline`} onClick={() => changeSport('Tennis')}>Tennis</button>
          {/* Add more sports as needed */}
        </div>
      </div>
      <div className="flex-grow p-4 w-full  overflow-y-auto dark:border border-solid">
        {selectedSport && (
          <div>
